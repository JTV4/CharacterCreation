...
preview_bridge.py
=================
Two-view preview render for the untextured arched bridge.  A pure side
view sells the arch profile; a 3/4 view sells the width + posts.
...
GLB_PATH = os.path.abspath("viewer/public/buildings/Bridge.glb")

WIDTH, HEIGHT = 1280, 520   # wide letterbox — the bridge is long-and-low
VIEWS = [
    # Pure profile from +X so the arch silhouette reads cleanly against
    # the neutral backdrop.  Camera direction is mostly +X with a hair
    # of +Z lift so the deck reads as a surface, not just a line.
    ("bridge_preview_side.png",
     Vector((1.00, 0.00, 0.08)).normalized()),
    # 3/4 view from the +X / +Y / +Z octant.  Shows the arch, the
    # planked deck, and both rows of posts + handrails in one frame.
    ("bridge_preview_3q.png",

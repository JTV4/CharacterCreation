"""
preview_boat_paddle.py
======================
Three-view preview render for the wooden canoe paddle: 3/4, top-down
(shows the blade + grip silhouette), and end-on-blade for scale.

Run:
  /Applications/Blender.app/Contents/MacOS/Blender --background \
      --python preview_boat_paddle.py
"""
...
GLB_PATH = os.path.abspath("viewer/public/buildings/BoatPaddle.glb")

# Paddle is long-and-thin, so a wide letterbox works best for the
# top-down + 3/4 views; the end-on-blade view uses a squarer frame.
WIDTH_LONG,  HEIGHT_LONG  = 1400, 520
WIDTH_SQUARE, HEIGHT_SQUARE = 700, 700

VIEWS = [
    # 3/4 from above, mostly along -Y — shows the whole paddle length,
    # the leaf-blade at the near end, and the T-grip at the far end.
    ("boat_paddle_preview_3q.png",
     Vector((0.35, -1.00, 0.55)).normalized(), WIDTH_LONG, HEIGHT_LONG),
    # Straight top-down (blade lying flat) — best for seeing the blade
    # outline + T-grip proportions.
    ("boat_paddle_preview_top.png",
     Vector((0.02, 0.02, 1.00)).normalized(), WIDTH_LONG, HEIGHT_LONG),
    # End-on-blade view: camera at the tip looking down the paddle
    # length — shows the blade → shaft → grip stack in silhouette.
    ("boat_paddle_preview_end.png",
     Vector((0.20, -1.00, 0.05)).normalized(), WIDTH_SQUARE, HEIGHT_SQUARE),
]
FRAME_MARGIN = 1.15

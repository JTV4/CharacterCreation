...
| Blender | 3.6+        | Body shell extraction (headless CLI) |
...
| `shell_gloves`     | Shell Gloves     | Forearms, hands, fingers| 6 mm      |
| `shell_lower_body` | Shell Lower Body | Hips, legs, feet        | 5 mm      |
| `shell_boots`      | Shell Boots      | Shins, feet, toes       | 15 mm     |
...
| `gloves`      | lowerarm, hands, all finger bones             | 6 mm      | Forearms and hands                 |
| `lower_body`  | pelvis, thighs, shins, feet                   | 5 mm      | Hips to ankles                     |
| `boots`       | shins, feet, toes                             | 15 mm     | Knee to toe, sits outside pants    |
...
- **Material** — Shells render with colored `MeshStandardMaterial` (per-slot
  color from `SLOT_COLORS`), `FrontSide` rendering, and `polygonOffset` to
  avoid Z-fighting with the base mesh.
- **Cache busting** — A module-level `_buildTimestamp` is appended to GLB URLs
  so updated shells are fetched after a hard refresh.

### Workflow: creating textured equipment

1. Run the extractor to generate body shells
2. Use the shells as base meshes for AI texturing (e.g. Meshy) — the shell
   geometry already conforms to the character, so generated textures will fit
3. Import the textured mesh back into the viewer as a standard equipment slot
4. The textured mesh inherits the shell's rigging and animates with the character

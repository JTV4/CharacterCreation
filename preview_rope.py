...
preview_rope.py
===============
Three-view preview for the coiled hemp rope: straight top-down (best
angle for showing the concentric flake pattern), a low 3/4 to sell the
tube's roundness with smooth shading, and a side view showing the flat
profile against the ground.

Run:
  /Applications/Blender.app/Contents/MacOS/Blender --background \
      --python preview_rope.py
"""
...
GLB_PATH = os.path.abspath("viewer/public/buildings/Rope.glb")

# Rope footprint is ~50 cm × 50 cm × 3 cm — square outputs are the
# right shape for the coil's silhouette; the side view uses a wide
# letterbox to make the 3 cm height readable.
WIDTH_SQ, HEIGHT_SQ   = 900, 900
WIDTH_WIDE, HEIGHT_WIDE = 1400, 500

VIEWS = [
    # Top-down: sells the concentric flake pattern.
    ("rope_preview_top.png",
     Vector((0.02, 0.02, 1.00)).normalized(), WIDTH_SQ, HEIGHT_SQ),
    # Low 3/4: shows the coil in perspective with tube roundness.
    ("rope_preview_3q.png",
     Vector((0.65, -0.65, 0.40)).normalized(), WIDTH_SQ, HEIGHT_SQ),
    # Pure side: proves it sits flat on the ground, single layer.
    ("rope_preview_side.png",
     Vector((0.00, -1.00, 0.08)).normalized(), WIDTH_WIDE, HEIGHT_WIDE),
]
FRAME_MARGIN = 1.15
